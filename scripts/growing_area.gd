...
func _process(delta):
	# Example: Checking if a reference is valid before using i

	if plant_growing == false:
		plant = Global.plant_selected
	if has_method("_on_area_2d_input_event"):
		emit_signal("input_event")
	else:
	# Print a warning message indicating the method is missing
		pass
...
func _on_area_for_plant_area_entered(area):

		if plant_growing == true:
				pass

		elif plant_growing != true:
